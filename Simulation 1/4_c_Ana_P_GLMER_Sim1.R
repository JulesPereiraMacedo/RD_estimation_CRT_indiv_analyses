...
for (n in Scen) {
  ...
  write.table(name_poisslog,
              file = paste(P_res_file_glmer,"/Data_output_Poiss_log_Scenario_",n,".csv",sep = ""),
              row.names = FALSE,
              col.names = FALSE,
              sep = ";")
}
...
for (n in Scen) {
  ...
  res <- foreach(i = 1:itt,
                 ...
                 .packages = c("stats","arm","gee","geepack","spind","doBy","doRNG","doParallel","dplyr","here","geesmv","matrixcalc")) %dorng% fun_para_analyse_poiss_glmer(itt_para = i,
                                                                                                                                                                            n=n,
                                                                                                                                                                            Scenario_use = Scenario_use,
                                                                                                                                                                            Data_file = Data_file,
                                                                                                                                                                            P_res_file = P_res_file_glmer)
...
for (n in Scen) {
  assign(paste("Data_Poiss_S",n,sep = "_"),read.csv2(paste(P_res_file_glmer,"/Data_output_Poiss_log_Scenario_",n,sep = "",".csv")))
  ...
  if(length(itt_sim_not_done) >= 1){
    for (i in (1:length(itt_sim_not_done))) {
      ...
      write.table(res_poisslog,file = paste(P_res_file_glmer,paste("Data_output_Poiss_log_Scenario_",sep = "",n,".csv"),sep = "/"),append = TRUE,row.names = FALSE,col.names = FALSE,sep = ";",dec = ",")
    }
  }
}
