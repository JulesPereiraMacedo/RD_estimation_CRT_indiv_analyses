...
# Create a file where the results will be stock for all scenarii
# We recommend to create this file in the Base_file

Resu_file_gee = paste(Base_file,"/data_output_gee",sep = "")
dir.create(Resu_file_gee)
...
dir.create(G_res_file_gee)
dir.create(B_res_file_gee)
dir.create(P_res_file_gee)

Resu_file_glmer = paste(Base_file,"/data_output_glmer",sep = "")
dir.create(Resu_file_glmer)
...
dir.create(G_res_file_glmer)
dir.create(B_res_file_glmer)
dir.create(P_res_file_glmer)


# Correction file for the function 'fun_cor_%method' in section '11-Function analyzing by methods'

dir.create(paste(Base_file,"/correction",sep = ""))
dir.create(paste(Base_file,"/correction/correction_Gauss",sep = "")) # we did not really need this file but just in case because convergence problem (infinite type) are not present with Gaussian distribution, so we use the 'fun_para_analyse_gauss' directly and not the corrected version 'fun_cor_gauss'
dir.create(paste(Base_file,"/correction/correction_Bin",sep = ""))
dir.create(paste(Base_file,"/correction/correction_Poiss",sep = ""))
...
# nb_cov_tot:   nb_cov + nb_cov_clus = number of covariates in 'data' (individual + cluster level)

#Output#
# Pred_Trt:     Prediction of the link function probabilities for each individuals considered in the intervention arm
# Pred_NoTrt:   Prediction of the link function probabilities for each individuals considered in the control arm

# ! CARE ! Your data set would be always as the Outcome First column, Arm as the second column, cluster id in the third column 
#          and then, your individual-level covariates and finally the cluster-level covariates. (Only covariates used in the formula!)
#          Also take care to have your model/fit, the formula of your model should be in same order as the column data.
#          ie. Data = Outcome|Arm|cluster|cov_ind.1|cov_ind.2|cov_clu.1 -> the formula has to be "Outcome ~ Arm + cov_ind.1 + cov_ind.2 + cov_clu.1"
...
list_prev = list(c(0.5,0.4),c(0.2,0.15),c(0.1,0.05))
